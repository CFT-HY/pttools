...
def validate(v: np.ndarray, w: np.ndarray, xi: np.ndarray, sol_type: SolutionType):
    if sol_type == SolutionType.SUB_DEF:
        validate_def(v, w, xi, sol_type)
    elif sol_type == SolutionType.HYBRID:
        validate_def(v, w, xi, sol_type)
        validate_shock(v, w, xi, sol_type)
    elif sol_type == SolutionType.DETON:
        validate_shock(v, w, xi, sol_type)


def validate_def(v: np.ndarray, w: np.ndarray, xi: np.ndarray, sol_type: SolutionType):
    i_wall = np.argmax(v)
    v_wall = xi[i_wall]
    v1p = v[i_wall-1]
    v2p = v[i_wall]
    ...
    w1 = w[i_wall-1]
    w2 = w[i_wall]
    validate2(v1p, v2p, v1w, v2w, w1, w2, Phase.BROKEN, Phase.SYMMETRIC, sol_type)


def validate_shock(v: np.ndarray, w: np.ndarray, xi: np.ndarray, sol_type: SolutionType):
    v_wall = xi[-2]
    v1p = v[-3]
    v2p = 0
    v1w = -relativity.lorentz(v1p, v_wall)
    ...
    if sol_type == SolutionType.DETON:
        ...
        phase2 = Phase.SYMMETRIC
    validate2(v1p, v2p, v1w, v2w, w1, w2, phase1, phase2, sol_type)


def validate2(v1p: float, v2p: float, v1w: float, v2w: float, w1: float, w2: float, phase1: Phase, phase2: Phase, sol_type: SolutionType):
    dev = boundary.junction_conditions_solvable(np.array([v2w, w2]), bag, v1w, w1, phase1, phase2)
