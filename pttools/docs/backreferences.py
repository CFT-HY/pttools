...
from pttools.docs.utils import MISSING, is_same_object, resolve_object

#: A code object of Sphinx-Gallery, which describes an object that is used in an example
type CodeObject = dict[str, tp.Any]


def ensure_imported(module: str, doc_modules: tp.Container[str]) -> None:
...
def defining_module(cobj: CodeObject, doc_modules: tp.Container[str]) -> str | None:
    ...
    """
    ensure_imported(cobj["module"], doc_modules=doc_modules)
...
def add_defining_modules(
        code_objects: dict[str, list[CodeObject]],
        doc_modules: tp.Container[str]) -> dict[str, list[CodeObject]]:
    ...
    for cobjs in code_objects.values():
        for cobj in list(cobjs):
            module = defining_module(cobj, doc_modules=doc_modules)
...
def patch_sphinx_gallery(doc_modules: tp.Container[str]) -> None:
    """Patch Sphinx-Gallery to also record the modules in which the objects are defined."""
    gen_rst.identify_names = gen_identify_names(doc_modules=doc_modules)
