...
# Todo: replace these with the PTPlot versions
...
def calc_ke_frac(xi_w: float, alpha_n: float):
    ...
    if xi_w < cs:
        ...
        k = (
            k_b_val +
            (xi_w - cs) * delta_k_val +
            ((xi_w - cs)/(xi_j_val - cs))**3 * (k_c_val - k_b -(xi_j_val - cs) * delta_k_val)
        )
    elif xi_w == xi_j_val:
        k = k_c(alpha_n)
    elif xi_w > xi_j_val:
        ...
        k_d_val = k_d(alpha_n)
        k = (
            ((xi_j_val - 1)**3 * xi_j_val**(5/2) * xi_w ** (-5/2) * k_c_val * k_d_val) /
            (((xi_j_val - 1)**3 - (xi_w-1)**3) * xi_j_val**(5/2) * k_c_val + (xi_w - 1)**3 * k_d_val)
        )
    elif xi_w > 0.85:
        k = k_d(alpha_n)
    else:
