...
class Spectrum:
    ...
    def f(self, z: np.ndarray = None):
        if z is None:
            z = self.y
        return omgw0.f(z=z, r_star=self.r_star, f_star0=self.f_star0)

    @functools.cached_property
    def f_star0(self) -> float:
        return omgw0.f_star0(
            T_n=self.bubble.Tn,
            g_star=self.g_star
        )

    def F_gw0(self, g0: float = omgw0.G0, gs0: float = omgw0.GS0) -> float:
        return omgw0.F_gw0(
            g_star=self.g_star,
            g0=g0,
            gs0=gs0,
            gs_star=self.bubble.model.gs(w=self.bubble.va_enthalpy_density, phase=bubble.Phase.BROKEN)
        )
    ...
    def omgw0(
            ...
            g0: float = omgw0.G0,
            gs0: float = omgw0.GS0,
            suppression: omgw0.SuppressionMethod = omgw0.SUP_METHOD_DEFAULT):
        if suppression == omgw0.SuppressionMethod.NONE:
            sup = 1
        else:
            sup = omgw0.get_suppression_factor(vw=self.bubble.v_wall, alpha=self.bubble.alpha_n)
        return self.F_gw0(g0=g0, gs0=gs0) * self.pow_gw * sup

    @functools.cached_property
    def source_lifetime_factor(self) -> float:
        r"""Source lifetime correction factor :giombi_2024_cs:`\ `, eq. 3.13"""
        ...
        if not np.isinf(self.source_duration):
            ret *= (1 - (1 + self.source_duration)**(-1 - 2*nu_gdh2024))
        return ret
